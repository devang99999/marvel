...
# 🌐 Web fallback (cached + monthly budget so SerpAPI free tier isn't exhausted)
def live_web_search_fallback(query):
    ...
    try:
        from serp_helper import get_serp_cached, save_serp_cached, serp_search

        # 1) Return cached scraped content if we have it
        doc = get_serp_cached(query)
        if doc and doc.get("content"):
            return doc["content"]

        # 2) Get search results (from cache or API if under budget)
        results = serp_search(query, api_key=SERPAPI_KEY, num=1)
        if not results:
            return None
        ...
        if content:
            save_serp_cached(query, results, content=content)
        return content
    except Exception as e:
